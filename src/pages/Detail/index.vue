    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <!-- 父子通信-props -->
          <!-- 这里直接给数据可能会报错。因为数据还没回来的时候，传递的数据为undefined，需要处理 -->
          <Zoom :skuImageList="stuImageList" />
          <!-- 小图列表：轮播图 -->
          <ImageList :skuImageList="stuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      // 产品价格
      prince(state) {
        return state.detail.prince;
      },
    }),
    // 父组件传递给子组件的数据
    stuImageList() {
      // 如果服务器数据没有回来，则返回一个空对象
      return this.skuInfo.skuImageList || [];
    },
  },
  // 在detail组件挂载完毕后，派发action
  mounted() {
    // 注意派发action时，需要传递产品id【路由的params参数【输入占位元素名】】
    this.$store.dispatch("getGoodsInfo", this.$route.params.stuId);
  },
  methods: {
